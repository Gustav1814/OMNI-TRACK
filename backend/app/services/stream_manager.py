...
class CameraStream:
    ...
    def _capture_loop(self):
        ...
        fps_cap = max(1, min(int(self.config.fps_target), 240))
        frame_interval = 1.0 / fps_cap
        skip_counter = 0

        while self._running:
            loop_tick_start = time.monotonic()
            # Connect/reconnect
            ...
            if self.config.skip_frames > 0:
                ...
                if skip_counter % (self.config.skip_frames + 1) != 0:
                    # Still honor fps_target — otherwise we decode/pull as fast as the CPU allows.
                    elapsed = time.monotonic() - loop_tick_start
            ...
            if self.config.decode_imgsz and self.config.decode_imgsz > 0:
                h, w = frame.shape[:2]
                max_side = max(h, w)
                if max_side > self.config.decode_imgsz:
                    scale = self.config.decode_imgsz / max_side
                    nw = max(1, int(w * scale))
                    nh = max(1, int(h * scale))
                    frame = cv2.resize(frame, (nw, nh), interpolation=cv2.INTER_AREA)

            # Calculate actual FPS (wall clock, based on enqueued frames only)
            ...
            # Pace capture so we average ≤ fps_target frames read per second
            elapsed = time.monotonic() - loop_tick_start
            time.sleep(max(0, frame_interval - elapsed))
...
class StreamManager:
    ...
    async def get_frame_async(self, camera_id: int, timeout: float = 1.0) -> Optional[tuple]:
        """Get the latest frame from a camera (async)."""
        if camera_id not in self._streams:
            return None
        return await self._streams[camera_id].get_frame_async(timeout=timeout)
