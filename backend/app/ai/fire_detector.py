...
class FireDetector:
    ...
    # Anti-false-positive guards. These can be tuned via the constructor.
    DEFAULT_MIN_CONFIRM_FRAMES = 4    # Require this many consecutive hits before alerting
    DEFAULT_MIN_AREA_RATIO = 0.005    # Detection bbox must be ≥ 0.5% of the frame area
    DEFAULT_COOLDOWN_S = 30.0         # Per (camera, alert_type), suppress duplicate alerts
    ...
    def __init__(
        ...
        min_confirm_frames: int = DEFAULT_MIN_CONFIRM_FRAMES,
        ...
        self._min_confirm_frames = max(1, int(min_confirm_frames))
        self._min_area_ratio = max(0.0, float(min_area_ratio))
        self._cooldown_s = max(0.0, float(cooldown_s))
        # (camera_id, alert_type) -> consecutive hit count
        self._consecutive: Dict[tuple, int] = {}
        # (camera_id, alert_type) -> last emitted unix timestamp
        self._last_emit: Dict[tuple, float] = {}

        self._model_loaded = False
        # Model is lazy-loaded on first detection call (or explicit ensure_loaded)
    ...
    def ensure_loaded(self) -> bool:
        """Lazy-load the model on first use. Returns True if loaded successfully."""
    ...
    def _load_model(self) -> None:
        ...
        path = resolve_model_path(self.model_path) or self.model_path
        try:
    ...
    def detect(
        ...
        """
        # Lazy-load model on first detection call
        self.ensure_loaded()
