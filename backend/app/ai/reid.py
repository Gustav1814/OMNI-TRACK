...
class PersonReID:
    ...
    def extract_batch(self, crops: List[np.ndarray]) -> List[np.ndarray]:
        """
        Batch embedding extraction — runs all crops through the network in a single
        forward pass. Much faster than calling extract_embedding() in a loop
        (one forward pass vs. N forward passes).
        """
        if not crops:
            return []
        if self.model is None or self.transform is None:
            return [self._mock_embedding() for _ in crops]

        # Transform each crop (BGR→RGB→resize→normalize→tensor) then stack into a batch
        tensors = []
        for c in crops:
            try:
                rgb = c[:, :, ::-1].copy()
        ...
        # Handle any failed transforms by falling back to per-crop mock
        ...
        for k, i in enumerate(valid_idx):
            emb = features[k].flatten()
            norm = np.linalg.norm(emb)
            if norm > 0:
                emb = emb / norm
            out[i] = emb.astype(np.float32)
        # Fill any failed slots with mock
        return [e if e is not None else self._mock_embedding() for e in out]
