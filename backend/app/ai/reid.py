...
from app.ai.reid_backend import build_reid_backend
...
class PersonReID:
    ...
    def _load_model(self):
        backend_name = getattr(settings, "REID_BACKEND", "torchreid")
        try:
            self._backend = build_reid_backend(
                name=backend_name,
                model_name=self.model_name,
                weights=getattr(settings, "REID_WEIGHTS", ""),
                device=self.device,
            )
            # Compatibility fields for existing status checks.
            self.model = self._backend if self._backend.is_loaded else None
            logger.info(f"Loaded Re-ID backend: {backend_name} on {self.device}")
    ...
    def extract_embedding(self, person_crop: np.ndarray) -> np.ndarray:
        ...
        if self._backend is not None:
            return self._backend.extract(person_crop)
