...
@shelf_router.get("/zones", response_model=List[ShelfZone])
async def get_shelf_zones(
    request: Request,
    camera_id: Optional[int] = None,
    current_user: User = Depends(get_current_user),
):
    """Get all configured shelf zones (optionally filtered by camera)."""
    pipeline = getattr(request.app.state, "pipeline", None)
    if pipeline and pipeline.shelf_tracker:
        zones = pipeline.shelf_tracker.zones
...
@shelf_router.post("/zones")
async def add_shelf_zone(
    request: Request,
    zone: ShelfZone,
    current_user: User = Depends(get_current_user),
):
    """Add a new shelf zone for tracking engagement."""
    from app.ai.shelf_analytics import ShelfZoneConfig

    pipeline = getattr(request.app.state, "pipeline", None)
    if pipeline is None or pipeline.shelf_tracker is None:
        raise HTTPException(400, "Shelf analytics not enabled. Start pipeline with shelf enabled.")

    config = ShelfZoneConfig(
        zone_id=zone.zone_id,
        zone_name=zone.zone_name,
        bbox=tuple(zone.bbox),
        camera_id=zone.camera_id,
    )
    pipeline.shelf_tracker.add_zone(config)
    return {"status": "ok", "zone_id": zone.zone_id}


@shelf_router.delete("/zones/{zone_id}")
async def delete_shelf_zone(
    request: Request,
    zone_id: str,
    current_user: User = Depends(get_current_user),
):
...
@shelf_router.post("/zones/auto-create")
async def auto_create_zones(
    ...
    """
    Auto-create default shelf zones for a camera.
    Divides the frame horizontally into `zone_count` equal zones.
    """
    from app.ai.shelf_analytics import ShelfZoneConfig
    ...
    # Get camera resolution from stream manager stats
    resolution = (1920, 1080)  # default
    if pipeline.stream_manager:
        stream = pipeline.stream_manager._streams.get(camera_id)
        if stream:
            stats_res = stream._stats.resolution
            if stats_res and stats_res[0] > 0:
                resolution = stats_res

    w, h = resolution
    zone_height = h // zone_count
    created = []

    for i in range(zone_count):
        y1 = i * zone_height
        y2 = y1 + zone_height
        zone = ShelfZoneConfig(
            zone_id=f"cam{camera_id}_shelf_{i+1}",
            zone_name=f"Camera {camera_id} - Shelf {i+1}",
            bbox=(0, y1, w, y2),
            camera_id=camera_id,
        )
        pipeline.shelf_tracker.add_zone(zone)
        created.append({
            "zone_id": zone.zone_id,
            "zone_name": zone.zone_name,
            "bbox": zone.bbox,
    ...
    return {"status": "ok", "camera_id": camera_id, "zones_created": created}


@shelf_router.get("/per-camera")
async def get_shelf_per_camera(
    request: Request,
    current_user: User = Depends(get_current_user),
):
    """Get shelf engagement data per camera (active engagements)."""
    snapshot = _snapshot(request)
    per_camera = (snapshot.get("shelf") or {}).get("per_camera") or {}
    return per_camera


@shelf_router.get("/events")
async def get_shelf_events(
    ...
    """
    Recent pick / put-back events from the shelf-activity detector.
    Newest first. Optionally filter by camera_id.
    """
    snapshot = _snapshot(request)
    events = (snapshot.get("shelf") or {}).get("events") or []
    if camera_id is not None:
        events = [e for e in events if e.get("camera_id") == camera_id]
    return events[: max(1, int(limit))]


@shelf_router.get("/items")
async def get_shelf_items(
    ...
    """
    Latest non-person items detected per camera (generic COCO retail
    classes, or custom product classes if a custom model is loaded).
    """
    snapshot = _snapshot(request)
    items_by_cam = (snapshot.get("shelf") or {}).get("items_by_camera") or {}
    if camera_id is not None:
        # Keys may be int or str depending on JSON round-trips.
        for k, v in items_by_cam.items():
            if int(k) == int(camera_id):
                return v
        return []
    return items_by_cam


@shelf_router.get("/cameras")
async def get_active_cameras(
    request: Request,
    current_user: User = Depends(get_current_user),
):
    """Get cameras that are currently active in the pipeline (not DB)."""
    snapshot = _snapshot(request)
    zones = (snapshot.get("cameras") or {}).get("zones") or {}
    return [
        {"id": int(cam_id), "zone": zone}
