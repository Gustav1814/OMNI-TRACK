...
pipeline = ProcessingPipeline(
    ...
    enable_fire=getattr(settings, "ENABLE_FIRE", False),  # Lazy: only loads when enabled
)
...
async def stream_camera_live(
    ...
    async def generate():
        try:
            ...
            # Browser closed the MJPEG stream — normal, suppress noise.
            return
        except Exception as e:
            logger.debug(f"MJPEG stream cam {camera_id} ended: {e}")
            return
...
async def _pipeline_to_websocket(results, global_state):
    ...
    for cam_id, result in results.items():
        ...
        # Shelf pick / put-back events (one push per event so the UI
        # ticker updates instantly without waiting for a poll cycle).
        for ev in (getattr(result, "shelf_events", None) or []):
            try:
                await broadcast.push_shelf_event(ev)
            except Exception:
                pass
    ...
    # Cross-camera Re-ID handoffs (same person moving between cameras)
    try:
        for ev in pipeline.drain_cross_camera_matches():
            await broadcast.push_reid_match(
                ev["global_id"],
                ev["current_camera"],
                ev["previous_camera"],
            )
