...
export const reidAPI = {
    ...
    active: (windowSeconds = 15) => api.get('/reid/active', { params: { window_seconds: windowSeconds } }),
    status: () => api.get('/reid/status'),
    toggle: (enabled) => api.post('/reid/toggle', null, { params: { enabled } }),
    recentMatches: (limit = 20) => api.get('/reid/matches/recent', { params: { limit } }),
    snapshots: (globalId) => api.get(`/reid/snapshots/${globalId}`),
};

// URL for a person snapshot <img> tag (cache-busted by `v` so it refreshes)
export const reidSnapshotUrl = (globalId, cameraId, version = 0) =>
    `${API_BASE}/reid/snapshot/${encodeURIComponent(globalId)}/${cameraId}?v=${version}`;
...
export const shelfAPI = {
    ...
    listZones: (cameraId) => api.get(`/shelf/zones${cameraId ? `?camera_id=${cameraId}` : ''}`),
    addZone: (zone) => api.post('/shelf/zones', zone),
    deleteZone: (zoneId) => api.delete(`/shelf/zones/${zoneId}`),
    autoCreateZones: (cameraId, zoneCount = 3) =>
        api.post(`/shelf/zones/auto-create?camera_id=${cameraId}&zone_count=${zoneCount}`),
    cameras: () => api.get('/shelf/cameras'),
    events: (cameraId, limit = 50) =>
        api.get(`/shelf/events?limit=${limit}${cameraId ? `&camera_id=${cameraId}` : ''}`),
    items: (cameraId) =>
        api.get(`/shelf/items${cameraId ? `?camera_id=${cameraId}` : ''}`),
};
