...
export default function ShelfPage() {
    ...
    const chartData = zones.slice(0, 10).map((z, i) => ({
        ...
        score: Number(z.engagement_score),
        dwell: Number(z.avg_dwell_time),
    })).filter((d) => Number.isFinite(d.score) && Number.isFinite(d.dwell));
            ...
            <div className="two-col">
                <div className="card">
                    ...
                    <div style={{ display: 'grid', gap: 6, maxHeight: 360, overflow: 'auto' }}>
                        {zones.map((z, idx) => (
                            <div key={`${z.zone_id}-${z.rank ?? idx}`} style={{
                                display: 'grid',
                                ...
                                <span style={{ fontSize: 12 }}>{Number(z.avg_dwell_time ?? 0).toFixed(1)}s</span>
                                <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--accent-gold)' }}>
                                    {Number(z.engagement_score ?? 0).toFixed(1)}
                                </span>
                            </div>
                        ))}
                    </div>
