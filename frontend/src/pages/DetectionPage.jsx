...
import {
    PlayCircle, StopCircle, Plus, Upload, Video, Circle, Square, RefreshCw, Users,
} from 'lucide-react';
import {
    detectionAPI, pipelineAPI, footageAPI, liveStreamUrl, modelAPI, reidAPI,
} from '../services/api';
...
export default function DetectionPage() {
    ...
    const { data: pipeState, refresh: refreshPipeline } = useLivePoll(
        () => pipelineAPI.status(), { intervalMs: 12000 }
    );
    const { data: detStatus, refresh: refreshDet } = useLivePoll(
        () => detectionAPI.status(), { intervalMs: 10000 }
    );
    const { data: recStatus, refresh: refreshRec } = useLivePoll(
        () => detectionAPI.recordingStatus(), { intervalMs: 15000 }
    );
    const { data: footage, refresh: refreshFootage } = useLivePoll(
        () => footageAPI.list(), { intervalMs: 30000 }
    );
    const { data: modelsData, refresh: refreshModels } = useLivePoll(
        () => modelAPI.list(), { intervalMs: 120000 }
    );
    const { data: reidStatus, refresh: refreshReid } = useLivePoll(
        () => reidAPI.status(), { intervalMs: 15000 }
    );

    const toggleReid = async () => {
        try {
            const newState = !(reidStatus?.enabled ?? true);
            await reidAPI.toggle(newState);
            await refreshReid();
        } catch (e) {
        ...
        <div className="page-scroll">
            <div className="page-header">
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <label
                        title={`Re-ID assigns global IDs across cameras. Gallery: ${reidStatus?.gallery_size ?? 0} embeddings.`}
                        style={{
                            display: 'inline-flex',
                            alignItems: 'center',
                            gap: 6,
                            ...
                            background: reidStatus?.enabled ? 'rgba(34, 197, 94, 0.12)' : 'var(--bg-glass)',
                            cursor: 'pointer',
                            fontSize: 12,
                            fontWeight: 500,
                        }}
                    >
                        <input
                            type="checkbox"
                            checked={!!reidStatus?.enabled}
                            onChange={toggleReid}
                            style={{ cursor: 'pointer' }}
                        />
                        <Users size={12} />
                        Re-ID {reidStatus?.enabled ? 'ON' : 'OFF'}
                        {reidStatus?.gallery_size > 0 && (
                            <span style={{ color: 'var(--text-secondary)' }}>· {reidStatus.gallery_size}</span>
                        )}
                    </label>
