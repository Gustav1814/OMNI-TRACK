...
from services.model.cfgs.stage1.number_plate_detector import NumberPlateDetectorSTAGE1
...
def _ensure_weights_path() -> Path:
    weights_path = Path(__file__).resolve().parents[3] / "inferenced_weights" / "lpd.pt"
...
def main(video_url: str = VIDEO_URL_DEFAULT) -> None:
    ...
    pipeline = ModelPipeline(
        model_configs=[
            PipeStructure(
                model=NumberPlateDetectorSTAGE1(
                    model_path=str(weights_path),
                    model_id="npd",
                    tag=["license_plate", "number_plate", "plate"],
                    device=device,
                ),
                model_id="npd",
                order=0,
