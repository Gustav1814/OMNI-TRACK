...
VIDEO_URL_DEFAULT = "https://ai-public-videos.s3.us-east-2.amazonaws.com/Inferenced+Videos/CCTV_Office_Scene_Generation.mp4"


def _ensure_weights_path() -> Path:
    weights_path = Path(__file__).resolve().parents[3] / "inferenced_weights" / "yolov8n.pt"
...
def main(video_url: str = VIDEO_URL_DEFAULT) -> None:
        ...
        model_configs=[
            PipeStructure(
                model=GeneralObjectDetectorStage1(
                    ...
                    model_id="yolov8n",
                    tag=["laptop", "mouse", "tv", "remote", "keyboard"],
                ),
                model_id="yolov8n",
                order=0,
                lead_by="",
    ...
    try:
        while True:
            ...
            cv2.imshow("Facing DIrection", frame)
