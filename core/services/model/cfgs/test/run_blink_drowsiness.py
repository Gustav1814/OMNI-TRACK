...
# from services.model.cfgs.stage1.number_plate_detector import NumberPlateDetector
...
from services.model.cfgs.stage1.general_pe_detector import GeneralPEStage1
from services.visualization.interface.iannotation_renderer import IAnnotationRenderer

# from services.model.cfgs.stage2.ocr import OCR
from services.visualization.pose_annotation_renderer import PoseAnnotationRenderer

VIDEO_URL_DEFAULT = "https://ai-public-videos.s3.us-east-2.amazonaws.com/Raw+Videos/Driver+drowsiness+detection+-+video+num+1.mp4"


def _ensure_weights_path() -> Path:
    weights_path = Path(__file__).resolve().parents[3] / "inferenced_weights" / "blink_drowse.pt"
...
def main(video_url: str = VIDEO_URL_DEFAULT) -> None:
    ...
    pipeline = ModelPipeline(
        model_configs=[
            ...
            PipeStructure(
                model=GeneralPEStage1(
                    model_path=str(weights_path),
                    model_id="blinking_drowsing",
                    tag=[
                        "black eyes l",
                        "black eyes r",
                        "leftbotton",
    ...
    try:
        while True:
            ...
            cv2.imshow("Blink_Drowsiness", frame)
